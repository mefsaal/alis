...
function partition() {
    ...
    if [ "$PARTITION_MODE" == "auto" ]; then
        ...
        PARTITION_PARTED_UEFI="mklabel gpt mkpart ESP fat32 1MiB 1GiB mkpart root $PARTITION_PARTED_FILE_SYSTEM_TYPE 1GiB 100% set 1 esp on"
        PARTITION_PARTED_BIOS="mklabel msdos mkpart primary ext4 4MiB 1GiB mkpart primary $PARTITION_PARTED_FILE_SYSTEM_TYPE 1GiB 100% set 1 boot on"
...
function bootloader_grub() {
    ...
    arch-chroot /mnt sed -i -E 's/GRUB_CMDLINE_LINUX_DEFAULT="(.*) quiet"/GRUB_CMDLINE_LINUX_DEFAULT=" radeon.cik_support=0 radeon.si_support=0 amdgpu.cik_support=1 amdgpu.si_support=1 amdgpu.dc=1"/' /etc/default/grub
    ...
    if [ "$BIOS_TYPE" == "uefi" ]; then
        ...
        arch-chroot /mnt grub-install --target=x86_64-efi --bootloader-id="Arch Linux" --efi-directory=$ESP_DIRECTORY --recheck --removable
...
function desktop_environment_kde() {
    pacman_install "plasma-meta plasma-wayland-session dolphin dolphin-plugins kate partitionmanager spectacle ark konsole"
